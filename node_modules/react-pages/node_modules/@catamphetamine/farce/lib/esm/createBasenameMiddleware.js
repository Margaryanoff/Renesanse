...
export default function createBasenameMiddleware({
  basename
}) {
  ...
  const pathnamePrefix = basename.slice(-1) === '/' ? basename.slice(0, -1) : basename;
  return createLocationMiddleware({
    makeLocationDescriptor: location => Object.assign({}, location, {
      pathname: `${pathnamePrefix}${location.pathname}`
    }),
    makeLocation: location => Object.assign({}, location, {
      pathname: // `farce` had a bug here:
      ...
      location.pathname.indexOf(pathnamePrefix) === 0 ? location.pathname.slice(pathnamePrefix.length) || '/' : null
    })
  });
}