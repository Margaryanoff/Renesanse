...
function runListenerEntry({
  ...
  try {
    ...
    if (process.env.NODE_ENV !== "production") console.warn(`Ignoring navigation listener \`${listener.name}\` that failed with \`${e}\`.`);
    result = null;
  }
...
function runListenerEntries(listenerEntries, location, callback) {
  ...
  return runListenerEntry(listenerEntries[0], location, result => result != null ? callback(result) : runListenerEntries(listenerEntries.slice(1), location, callback));
}
...
function runAllowNavigation(listenerEntries, location, callback) {
  return runListenerEntries(listenerEntries, location, result => callback(maybeConfirm(result)));
}
...
export default function createNavigationListenerMiddleware() {
  ...
  function onBeforeUnload(event) {
    const syncResult = runListenerEntries(listenerEntries, null, result => result);
  ...
  function addListener(listener, {
    ...
    return () => {
      listenerEntries = listenerEntries.filter(item => item !== listenerEntry);
  ...
  function navigationListenerMiddleware({
    dispatch
  }) {
    return next => action => {
      const {
      ...
      switch (type) {
        case ActionTypes.NAVIGATE:
          return runAllowNavigation(listenerEntries, payload, allowNavigation => {
            if (!allowNavigation) {
              return null;
        ...
        case ActionTypes.UPDATE_LOCATION:
          {
            ...
            if (payload.delta == null) {
              return runAllowNavigation(listenerEntries, payload, allowNavigation => allowNavigation ? next(action) : null);
            }
            ...
            const syncResult = runListenerEntries(listenerEntries, payload, result => {
              if (sync) {
                return result;
              }
