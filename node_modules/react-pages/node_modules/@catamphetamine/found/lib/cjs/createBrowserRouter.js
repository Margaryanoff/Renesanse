"use strict";

var _interopRequireDefault = require("@babel/runtime/helpers/interopRequireDefault");
exports.__esModule = true;
exports.default = createBrowserRouter;
var _BrowserProtocol = _interopRequireDefault(require("@catamphetamine/farce/BrowserProtocol"));
var _react = _interopRequireDefault(require("react"));
var _createFarceRouter = _interopRequireDefault(require("./createFarceRouter"));
var _resolver = _interopRequireDefault(require("./resolver"));
var _jsxRuntime = require("react/jsx-runtime");
function createBrowserRouter(options) {
  const Router = (0, _createFarceRouter.default)(Object.assign({}, options, {
    historyProtocol: new _BrowserProtocol.default()
  }));
  function BrowserRouterInstance(props) {
    // @ts-ignore TODO: resolver will be always overwritten
    return /*#__PURE__*/(0, _jsxRuntime.jsx)(Router, Object.assign({
      resolver: _resolver.default
    }, props));
  }
  return BrowserRouterInstance;
}
module.exports = exports.default;