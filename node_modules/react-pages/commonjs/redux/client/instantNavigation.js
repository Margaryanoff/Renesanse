...
exports.isNavigationWithInstantBackAbility = isNavigationWithInstantBackAbility;
exports.resetInstantNavigationChain = resetInstantNavigationChain;
exports.setInstantBackAbilityFlagForThisNavigation = setInstantBackAbilityFlagForThisNavigation;
...
var debug = function debug() {
  if ((0, _context.getFromContext)('Navigation/InstantNavigationDebug')) {
    var _console;
...
function addInstantBack(nextLocation, previousLocation, nextLocationRouteComponents, previousLocationRouteComponents) {
  var chain = getInstantNavigationChain();
  debug('Add a new entry to instant navigation chain', {
    chain: chain,
    nextLocation: nextLocation,
    previousLocation: previousLocation,
    nextLocationRouteComponents: nextLocationRouteComponents,
    previousLocationRouteComponents: previousLocationRouteComponents
  });
...
function isInstantTransition(fromLocation, toLocation) {
  var chain = getInstantNavigationChain();
  var isInstant = indexOfByKey(chain, getLocationKey(fromLocation)) >= 0 && indexOfByKey(chain, getLocationKey(toLocation)) >= 0;
...
function getInstantNavigationChain() {
  if (!(0, _context.getFromContext)('Navigation/InstantNavigationChain')) {
    (0, _context.setInContext)('Navigation/InstantNavigationChain', []);
  }
  return (0, _context.getFromContext)('Navigation/InstantNavigationChain');
}
function getInstantNavigationChainIndex() {
  if (!(0, _context.getFromContext)('Navigation/InstantNavigationChainIndex')) {
    (0, _context.setInContext)('Navigation/InstantNavigationChainIndex', -1);
  }
  return (0, _context.getFromContext)('Navigation/InstantNavigationChainIndex');
}
function setInstantNavigationChain(value) {
  (0, _context.setInContext)('Navigation/InstantNavigationChain', value);
}
function setInstantNavigationChainIndex(value) {
  (0, _context.setInContext)('Navigation/InstantNavigationChainIndex', value);
}
...
function updateInstantNavigationChainIndex(location) {
  ...
  while (i < chain.length) {
    if (chain[i].key === getLocationKey(location)) {
      ...
      debug('Update index in instant navigation chain', {
        chain: chain,
        location: location,
        i: i
      });
...
function wasInstantNavigation() {
  return typeof window !== 'undefined' && (0, _context.getFromContext)('Navigation/WasInstant') === true;
}
function isInstantBackAbleNavigation() {
  return typeof window !== 'undefined' && (0, _context.getFromContext)('Navigation/IsInstantBack');
}
function setInstantNavigationFlag(value) {
  (0, _context.setInContext)('Navigation/WasInstant', value);
}
 ...
 * @return
 */
function setInstantBackAbilityFlagForThisNavigation(instantBack) {
  // This flag is being read in `./redux/middleware/router.js`
  (0, _context.setInContext)('Navigation/IsInstantBack', instantBack);
  ...
  setTimeout(function () {
    (0, _context.clearInContext)('Navigation/IsInstantBack');
  }, 0);
}
function isNavigationWithInstantBackAbility() {
  return (0, _context.getFromContext)('Navigation/IsInstantBack');
}
